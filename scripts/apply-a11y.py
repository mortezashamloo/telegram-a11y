...
def patch_reactions_as_menu() -> None:
    ...
    new_toggle = (
        ...
        "                           ...(truncated 20684 characters)...tivity ghost-mode regex found no matches")
        return
...
def patch_add_a11y_strings() -> None:
    strings = {
        ...
        "A11yVoiceQualityLabel": ("Voice quality: %1$s", "کیفیت پیام صوتی"),
...
def patch_longpress_menu_order() -> None:
    """
    Accessibility-fork: fix double "Reactions" menu item + correct order in long-press menu
    (Reactions -> Bot Buttons -> Select before Go to first message).
    """
    ca = JAVA / "org/telegram/ui/ChatActivity.java"
    if not ca.exists():
        print("WARN: ChatActivity missing (longpress menu order)")
        return
    t = ca.read_text(encoding="utf-8")
    if "a11y-fork: reactions menu item" not in t or "a11y-fork: bot buttons menu" not in t:
        print("WARN: Reactions or Bot Buttons not found (order patch skipped)")
        return

    # اضافه کردن گزینه Select اگر وجود نداشت
    if "Select" not in t:
        t = re.sub(
            r"items\.add\(LocaleController\.getString\(R\.string\.Select\)\);",
            "items.add(LocaleController.getString(R.string.Select));\n                options.add(203);\n                icons.add(R.drawable.msg_select);\n                // a11y-fork: longpress menu order - Select last before Go to first message",
            t,
            flags=re.IGNORECASE,
        )

    # مطمئن شدن از اینکه گزینه Select همیشه آخرین گزینه قبل از Go to first message باشه
    if "id == a11y_go_to_first_message" in t:
        t = t.replace(
            "                } else if (id == a11y_go_to_first_message) {",
            """                } else if (id == 203) {  // Select (a11y-fork: longpress menu order)
                    // Select is now the last option before Go to first message
                    selectedObject = null;
                    selectedObjectGroup = null;
                    return;
                } else if (id == a11y_go_to_first_message) {""",
    ...
    ca.write_text(t, encoding="utf-8")
    print("Longpress menu order (Reactions + Bot Buttons + Select) OK")
