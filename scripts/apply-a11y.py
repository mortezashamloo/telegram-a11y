...
def main() -> int:
    steps = [
        ("AppName", patch_app_name),
        ("A11yConfig.java install", install_a11y_config),
        ("RadialProgress announce", patch_radial_progress),
        ("DialogCell name-then-type", patch_dialogcell_name_then_type),
        ("Hide share/comment button", patch_hide_share_and_comment),
    ]
    failed = []
    for label, fn in steps:
        try:
            ...
            failed.append(label)
    if failed:
        print("apply-a11y.py finished with errors in:", ", ".join(failed), file=sys.stderr)
        return 1
    print("apply-a11y.py: all steps completed")
    return 0
